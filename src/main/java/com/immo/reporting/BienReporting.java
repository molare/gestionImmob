 ...
 */
public class BienReporting {

    private List<Bien> reportData;
    ...
    public BienReporting(List<Bien> reportData, String userName) {
    ...
    public JasperReportBuilder build(HttpServletRequest request) throws DRException {
        ...
        TextColumnBuilder<String> foncierColumn = col.column("Numero foncier", "nberFoncier", type.stringType());
        TextColumnBuilder<String> lotColumn = col.column("Numero lot", "nberlot", type.stringType());
        TextColumnBuilder<String> ilotColumn = col.column("Numero Ilot", "nberIlot", type.stringType());
        TextColumnBuilder<String> bienColumn = col.column("Nom bien", "designation", type.stringType());
        TextColumnBuilder<String> propertyColumn = col.column("Proprietaire", "propertyTransient", type.stringType());
        TextColumnBuilder<Double> coutColumn = col.column("Cout acquisition", "acquisitionCost", type.doubleType());
        TextColumnBuilder<String> dateColumn = col.column("Date creation", "dateTransient", type.stringType());
        TextColumnBuilder<String> cityColumn = col.column("Quartier", "cityTransient", type.stringType());
        ...
        subreport
                ...
                .columns(dateColumn,bienColumn, typeColumn,propertyColumn, coutColumn,ilotColumn,lotColumn,foncierColumn,superficyColumn,cityColumn)
    ...
    private JRDataSource createDataSource() {
        DRDataSource dataSource = new DRDataSource("dateTransient","designation","propertyTransient","acquisitionCost","typeTransient","superficy","cityTransient","nberIlot","nberFoncier","nberlot");
        for(Bien e : reportData){
            dataSource.add(e.getDateTransient(),e.getDesignation(), e.getPropertyTransient(), e.getAcquisitionCost(),e.getTypeTransient(), e.getSuperficy(), e.getCityTransient(), e.getNberIlot(), e.getNberFoncier(), e.getNberlot());
        }
        return dataSource;
    }
...
/*            addCustomerAttributeTitle(list, " Code : "+reportData.get(0).getCustomerCodeTransient());
    ...
    private ComponentBuilder<?, ?> createTitreComponent() {
        ...
        addCustomerAttributeTitle2(list, "Liste bien");
