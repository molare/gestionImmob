 ...
 */
public class StatePayRollReporting {
    ...
    public StatePayRollReporting(List<PayRoll> reportData, String userName) {
    ...
    public JasperReportBuilder build(HttpServletRequest request) throws DRException {
        ...
        TextColumnBuilder<Double> totalColumn = col.column("Total (FCFA)", "totalTransient", type.doubleType());
       // TextColumnBuilder<String> typeColumn = col.column("Type bien", "typeTransient", type.stringType());
        TextColumnBuilder<Double> cautionColumn = col.column("Caution (FCFA)", "cautionTransient", type.doubleType());
        TextColumnBuilder<Double> restCautionColumn = col.column("Caution restant (FCFA)", "restCautionTransient", type.doubleType());
        ...
        subreport
                ...
                .columns(/*dateColumn,*/ bienColumn, locativeColumn, locaterColumn, amountColumn, cautionColumn, restCautionColumn, totalColumn)
                .setColumnTitleStyle(titleStyle)
                .subtotalsAtSummary(sbt.text("Total", bienColumn).setLabelPosition(Position.LEFT), sbt.text("", locativeColumn), sbt.text("", locaterColumn),sbt.sum(amountColumn),sbt.sum(cautionColumn),sbt.sum(restCautionColumn),sbt.sum(totalColumn)).setSubtotalStyle(someStyle)
                .noData(cmp.text("Pas de reponse a votre requete").setStyle(stl.style().setForegroundColor(Color.RED).setFontSize(14)).setHorizontalAlignment(HorizontalAlignment.CENTER))
        ...
        setSubtotalStyle(someStyle)*/
                ...
                .title(
                        ...
                        cmp.verticalGap(30),
                        cmp.horizontalFlowList(createTitreComponent(), cmp.gap(20, 47)),
                        cmp.verticalGap(30)
                )
    ...
    private JRDataSource createDataSource() {
        DRDataSource dataSource = new DRDataSource("dateTransient","bienTransient","locativeTransient","locaterTransient","amount","cautionTransient","restCautionTransient","totalTransient");
        for(PayRoll e : reportData){
            dataSource.add(e.getDateTransient(),e.getBienTransient(), e.getLocativeTransient(), e.getLocaterTransient(),e.getAmount(), e.getCautionTransient(), e.getRestCautionTransient(), e.getTotalTransient());
    }
    return dataSource;
    }
...
/*            addCustomerAttributeTitle(list, " Code : "+reportData.get(0).getCustomerCodeTransient());
    ...
    private ComponentBuilder<?, ?> createTitreComponent() {
        ...
        addCustomerAttributeTitle2(list, "RAPPORT DES PAIEMENTS LOYERS");
